...
`;
      ...
      expect(
        result.dependencies.some((dep) => dep.endsWith("sections/intro.md"))
      ).toBe(true);
      expect(
        result.dependencies.some((dep) =>
          dep.endsWith("sections/conclusion.md")
        )
      ).toBe(true);
      expect(
        result.dependencies.some((dep) => dep.endsWith("guides/advanced.md"))
      ).toBe(true);
      expect(
        result.dependencies.some((dep) =>
          dep.endsWith("guides/getting-started.md")
        )
      ).toBe(true);
    });
...
`;
      ...
      expect(result.errors.some((e) => e.message.includes("frontmatter"))).toBe(
        true
      );
